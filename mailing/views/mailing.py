...
class MailingListView(LoginRequiredMixin, ListView):
    model = Mailing
    permission_required = "schedule.can_view_all_mailings"
    ...
    def get_queryset(self):
        ...
        if user.has_perm("schedule.can_view_all_mailings") or user.is_superuser:
...
class MailingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["text"].queryset = MailingText.objects.filter(
            company=company
        )
        return form
...
class MailingUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["clients"].queryset = Client.objects.filter(company=company)
        form.fields["message"].queryset = MailingText.objects.filter(
            company=company
        )
        return form
